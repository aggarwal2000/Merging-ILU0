...
__device__ void ApplyPreconditioner_ISAI(const int num_rows , const int* const aiA_row_ptrs, 
    const int* const aiA_col_idxs , const double* const aiA_values,  const double* const vec_shared, double* const vec_hat_shared)
{

    // vec_hat = precond * vec
    SpMV(num_rows, aiA_row_ptrs, aiA_col_idxs, aiA_values, vec_shared, vec_hat_shared);

}



__global__ void KernelBatchedPreconditionedBiCGSTABGeneralISAI(const int num_rows, const int num_nz, const int num_pages, const int* const row_ptrs, 
    const int* const col_inds, const double* const vals_mat, const double* const vals_rhs, double* const vals_ans,
    const int aiA_nnz , const int* const aiA_row_ptrs, const int* const aiA_col_idxs, const double* const aiA_vals ,
            ...
            else
            {
                 /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Then can start iterating ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
                    ...
                    while(iter < MAX_ITER)
                    {
                        ...
                        ApplyPreconditioner_ISAI(num_rows, aiA_row_ptrs, aiA_col_idxs, aiA_vals + page_id * aiA_nnz, p_shared, p_hat_shared);
                        ...
                        ApplyPreconditioner_ISAI(num_rows, aiA_row_ptrs, aiA_col_idxs, aiA_vals + page_id * aiA_nnz, s_shared, s_hat_shared);
                        __syncthreads();
...
int Batched_BiCGSTAB_General_ISAI_Gpu_helper(const PagedCSRMatrices & A_pages,const PagedVectors& b_pages,PagedVectors & x_pages, SolverResults & solver_results,
     double & PST, double & IST, double & OET )
{
    ...
    PagedCSRMatrices aiA_pages;
    
    GenerateISAI_gpu(aiA_pages, A_pages, mat_type::general,1);	
    ...
    KernelBatchedPreconditionedBiCGSTABGeneralISAI<<< grid_solver, block , 0  >>>(A_pages.GetNumRows(), A_pages.GetNumNz(), A_pages.GetNumPages(),
    ...
    aiA_pages.GetNumNz(), aiA_pages.GetPtrToGpuRowPtrs(), aiA_pages.GetPtrToGpuColInd(), aiA_pages.GetPtrToGpuValues(), 
     ...
     double & PST, double & IST, double & OET  )
{
    ...
    success_code = Batched_BiCGSTAB_General_ISAI_Gpu_helper(A_pages,b_pages,x_pages, solver_results, PST, IST, OET);

    std::string solution_file;

    if(is_scaled == true)
        solution_file = "x_scaled_gpu_general_isai_bicgstab.mtx";
    else
        solution_file = "x_gpu_general_isai_bicgstab.mtx";
