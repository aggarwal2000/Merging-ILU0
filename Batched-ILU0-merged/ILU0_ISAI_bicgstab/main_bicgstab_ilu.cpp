...
{	
	...
	if(argc < 7)
	{
		printf("\nFormat: ./a.out [category name] [problemsize] [1 for scaled] [0 for conventional ilu, 1 for par ilu] [ app num for conv ilu(for parilu, this will not be used) ]  [ num_iter for par ilu(for conventional ilu, this will not be used) ]   [<optional> file to store execution time(cuda implementation)]\n");
		exit(0);
	}
	...
	int app_num_conv_ilu = 1;

	if(is_parilu == true)
	{
		num_iter_par_ilu = std::stoi(argv[6]);
	}
	else
	{
		app_num_conv_ilu = std::stoi(argv[5]);
	}
	...
	for(int i=0; i< rounds; i++)
	{
		...
		app_num_conv_ilu, num_iter_par_ilu, PST, IST, OET);  
	...
	PrintSolverResults(subdir,solver_results, std::string("Results_ilu_bicgstab_sep_")  + specifics + category + std::string(".txt"));
